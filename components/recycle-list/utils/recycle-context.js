...
RecycleContext.prototype._recalculateSizeByProp = function (list, cb) {
  ...
  if (itemSize.cacheKey) {
    propValueMap = my.getStorageSync(itemSize.cacheKey) || []
  ...
  function sizeReady(rects) {
    ...
    if (itemSize.cacheKey) {
      my.setStorageSync(itemSize.cacheKey, propValueMap) // 把数据缓存起来
    }
    if (cb) {
      cb(sizeData)
    }
  ...
  if (calcNewItems.length) {
    ...
    this.page.setData(obj, () => {
      my.createSelectorQuery().selectAll('.' + itemSize.queryClass).boundingClientRect((rects) => {
        sizeReady(rects)
      }).exec()
    })
  } else {
